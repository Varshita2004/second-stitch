...
def payment_success_view(request):
    ...
    <html>
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h2>Thank You for Your Order, {customer.user.first_name}!</h2>
                </div>
                <p>We have received your order and it will be processed shortly. Below are your order details:</p>

                <div class="order-summary">
                    <h3>Order Summary</h3>
                    <ul class="product-list">
                        {''.join([f'<li>{product.name} - {product.price} USD</li>' for product in products])}
                    </ul>

                    <p class="total">Total: {len(products)} item(s)</p>
                </div>
                ...
                <div class="footer">
                    <p>Thank you for shopping with us! If you have any questions, please contact our customer service.</p>
                </div>
            </div>
        </body>
    </html>
    ...
    # Send the email with the HTML invoice
    if email:
        send_mail(
            subject="Your Order Invoice",
            message="This is a plain text fallback message.",  # Fallback text
    ...
    # Clear cookies and render the success page
...
from django.http import HttpResponse
from .models import NGO  # Assuming you have an NGO model to store the organization details

# View to list NGOs
def list_ngos(request):
    ngos = NGO.objects.all()  # Fetch all NGOs
    return render(request, 'ecom/ngos_list.html', {'ngos': ngos})

# View to handle donation form submission
from django.shortcuts import render, redirect, get_object_or_404
from .models import NGO, Donation

def donate_clothes(request, ngo_id):
    ...
    if request.method == 'POST':
        ...
        num_clothes = request.POST.get('num_clothes')

        # Concatenate address fields into a single string
        address = f"{flat_no}, {street}, {city}, {state}, {pincode}, {country}"

        # Save donation data
        Donation.objects.create(ngo=ngo, address=address, num_clothes=num_clothes)

        return redirect('thank_you')  # Redirect to a thank you page

    return render(request, 'ecom/donate_clothes.html', {'ngo': ngo})

# Thank you view
def thank_you(request):
    return render(request, 'ecom/thank_you.html')
